...
public class InterfacePrincipal {
    ...
    private static final Font FONT_TAB_TITLE = new Font("Segoe UI", Font.BOLD, 14); // Aumentado de volta, título será curto


    public static void main(String[] args) {
        try {
            // UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
            // Tentando Nimbus para uma aparência potencialmente melhor
    ...
    private static void aplicarEstiloBotao(JButton button) {
        ...
        // Removendo o listener de mouse para o hover se estiver causando problemas ou não desejado.
        // Se quiser manter, certifique-se que a lógica de repintura é robusta.
        // Por simplicidade, vamos remover por enquanto para focar no layout.
    ...
    // ... [mostrarTelaLogin, mostrarTelaRegistro, mostrarTelaPrincipal SEM ALTERAÇÕES NOVAS AQUI] ...
    // O código para mostrarTelaLogin, mostrarTelaRegistro, mostrarTelaPrincipal será omitido aqui para brevidade,
    // pois as mudanças principais serão nos painéis das abas. Assuma que eles estão como na última versão completa fornecida.
    // Apenas garanta que mostrarTelaPrincipal define um tamanho adequado para a janela.
    ...
    private static void mostrarTelaPrincipal() {
        ...
        telaPrincipalFrame.setMinimumSize(new Dimension(980, 720)); // Ajustado mínimo
        ...
        // Removido o listener de mouse customizado para o botão sair para consistência,
        // A menos que queira um efeito diferente para ele.
    ...
    // Método auxiliar para criar painéis de forma mais consistente
    private static JPanel criarPainelBase(String tituloDaAba) {
        ...
        TitledBorder titledBorder = BorderFactory.createTitledBorder(
            ...
            tituloDaAba, // Usar título curto aqui
            TitledBorder.LEFT, 
            TitledBorder.TOP,      
            FONT_TAB_TITLE, 
            ACADEMUS_MAIN_PURPLE
        );
        // Aumentando o padding externo para dar mais espaço ao redor do conteúdo da aba
    ...
    private static void adicionarCampoComLabel(JPanel painel, GridBagConstraints gbc, JLabel label, Component campo, int yPos) {
        ...
        gbc.weightx = 0.2; // Peso para coluna de labels
        painel.add(label, gbc);

        gbc.gridx = 1; gbc.gridy = yPos;
        gbc.gridwidth = GridBagConstraints.REMAINDER; // Ocupa o resto da linha
        gbc.anchor = GridBagConstraints.WEST;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.weightx = 0.8; // Peso para coluna de campos
    ...
    private static JPanel criarPainelAlunos() {
        JPanel painel = criarPainelBase("Alunos"); // Título CURTO para a borda
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(8, 8, 8, 8); // Espaçamento entre componentes
                                            // (era 10,10,10,10, ajustado para consistência)
        ...
        JButton carregarAlunoBtn = new JButton("Carregar Dados"); aplicarEstiloBotao(carregarAlunoBtn); // Texto encurtado
        ...
        gbc.gridx = 0; gbc.gridy = y; gbc.weighty = 1.0; gbc.gridwidth = 2; painel.add(new JLabel(""), gbc); // Preencher espaço vertical

        // Action Listeners (mantidos, mas com textos de JOptionPane agora padronizados pelo UIManager)
    ...
    private static JPanel criarPainelProfessores() {
        JPanel painel = criarPainelBase("Professores"); // Título CURTO
        GridBagConstraints gbc = new GridBagConstraints();
    ...
    private static JPanel criarPainelCursos() {
        JPanel painel = criarPainelBase("Cursos"); // Título CURTO
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(8, 8, 8, 8);

        JLabel nomeLabel = new JLabel("Nome do Curso:"); aplicarEstiloLabel(nomeLabel);
        JTextField nomeField = new JTextField(25); aplicarEstiloTextField(nomeField);
        JLabel cargaHorariaLabel = new JLabel("Carga Horária (h):"); aplicarEstiloLabel(cargaHorariaLabel); // Abreviado
        JTextField cargaHorariaField = new JTextField(10); aplicarEstiloTextField(cargaHorariaField);
        JLabel profRespLabel = new JLabel("Prof. Responsável:"); aplicarEstiloLabel(profRespLabel); // Abreviado
    ...
    private static JPanel criarPainelVinculacoes() {
        JPanel painel = criarPainelBase("Vinculações"); // Título CURTO
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(8, 8, 8, 8);
        ...
        JLabel profLabel = new JLabel("Selecionar Professor:"); aplicarEstiloLabel(profLabel); // Simplificado
        JComboBox<String> professorBox = new JComboBox<>(); aplicarEstiloComboBox(professorBox);
        
        JButton atualizarCombosBtn = new JButton("Atualizar Listas"); aplicarEstiloBotao(atualizarCombosBtn); // Encurtado
        ...
        JButton cadastrarBtn = new JButton("Vincular Selecionados"); aplicarEstiloBotao(cadastrarBtn); // Encurtado
        JButton listarBtn = new JButton("Listar Vinculações"); aplicarEstiloBotao(listarBtn); // Encurtado
